 ...
 *
 * Usage: php ESP32Reader.php --port=COM7 --url=http://127.0.0.1:8000
 */
class ESP32Reader
{
    private $port;
    ...
    /**
     * Read a single key from Laravel's .env file.
     */
    private function getLaravelEnvValue(string $key): ?string
    {
    ...
    public function processRfidData($data)
    {
        ...
        elseif (strpos($data, 'Sent to bridge:') !== false) {
        ...
        elseif (preg_match('/Card detected:\s*([A-F0-9]+)/i', $data, $matches)) {
        ...
        } elseif (preg_match('/^([A-F0-9]+)(:(\d+))?$/i', $data, $matches)) {
